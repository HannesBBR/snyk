...
function getPatchPrompts(vulns: AnnotatedIssue[], policy, options?): Prompt[] {
...
function getUpdatePrompts(vulns: AnnotatedIssue[], policy, options?): Prompt[] {
  ...
  // and `reduce` _really_ doesn't like when you change the array under
  // it's feet
  // TODO(kyegupov): rewrite this reduce into something more sensible
...
function generatePrompt(vulns: AnnotatedIssueWithGrouping[], policy, prefix: string, options): Prompt[] {
  if (!prefix) {
    prefix = '';
  }
  if (!vulns) {
  ...
  let prompts = vulns.map((vuln, i) => {
    ...
    const res: Prompt = {
      when(answers) {
        ...
        if (vuln.grouped && !vuln.grouped.main) {
          ...
          res2 = groupAnswer.filter((answer) => {
    ...
    res.default = (choices.map((choice, ii) => {
      return {i: ii, default: choice.default};
    }).filter((choice) => {
